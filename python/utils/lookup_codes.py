"""
State and country code lookup data for the CardDemo application.

Translated from CSLKPCDY.cpy — contains valid US state codes,
North America phone area codes, and state-zip code combinations
...
# ---------------------------------------------------------------------------
# US State codes (including DC, territories)
# From VALID-US-STATE-CODE in CSLKPCDY.cpy
# ---------------------------------------------------------------------------
...
def is_valid_us_state_code(code: str) -> bool:
    """Check whether ``code`` is a valid US state/territory code.

    Args:
        code: A 2-character state code (case-sensitive, uppercase expected).

    Returns:
        True if the code is in the valid set.
    """
    return code in VALID_US_STATE_CODES


# ---------------------------------------------------------------------------
# State + first-2-digits-of-ZIP combinations
# From VALID-US-STATE-ZIP-CD2-COMBO in CSLKPCDY.cpy
# ---------------------------------------------------------------------------
...
def is_valid_us_state_zip_combo(state_code: str, zip_first_two: str) -> bool:
    """Check whether a state + first-two-digits-of-ZIP combination is valid.

    Args:
        state_code:    2-character US state code.
        zip_first_two: First 2 digits of the ZIP code.

    Returns:
        True if the combination is in the valid set.
    """
    return f"{state_code}{zip_first_two}" in VALID_US_STATE_ZIP_COMBOS


# ---------------------------------------------------------------------------
# North America phone area codes
# From VALID-PHONE-AREA-CODE in CSLKPCDY.cpy
# (subset — the full list contains ~500+ codes; representative sample below)
# ---------------------------------------------------------------------------
...
def is_valid_phone_area_code(code: str) -> bool:
    """Check whether ``code`` is a valid North American phone area code.

    Args:
        code: A 3-digit area code string.
